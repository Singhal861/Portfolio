...
export default function VerbManager({ userName }: { userName: string }) {
  ...
  function openAdd() { setEditingId(null); setForm({ ...emptyVerb }); setResult(''); setError(''); setFormOpen(true); }
  function openEdit(verb: Verb) {
    setEditingId(verb.id);
    setForm(fields.reduce((current, field) => ({ ...current, [field.key]: verb[field.key] || '' }), {} as VerbFields));
    setResult(''); setError(''); setFormOpen(true);
  }
  function closeForm() { setFormOpen(false); setEditingId(null); setResult(''); setError(''); setForm({ ...emptyVerb }); }
  function clearForm() { setForm({ ...emptyVerb }); setError(''); setResult(''); }
  function updateField(field: keyof VerbFields, value: string) { setForm((current) => ({ ...current, [field]: value })); }
  ...
  async function saveVerb(event: FormEvent) {
    ...
    try {
      ...
      await loadVerbs(); setResult(editingId ? 'Verb updated successfully.' : 'Verb added successfully.');
  ...
  return (
    <>
      ...
      {loading ? <p className="empty-state">Loading your verbs...</p> : <div className="table-wrap"><table><thead><tr><th>S.No</th>{fields.map((field) => <th key={field.key}>{field.label}</th>)}<th>Actions</th></tr></thead><tbody>{verbs.map((verb) => <tr key={verb.id}><td>{verb['S.No']}</td>{fields.map((field) => <td key={field.key}>{verb[field.key]}</td>)}<td className="action-cell"><button type="button" className="icon-action" aria-label="Edit verb" title="Edit verb" onClick={() => openEdit(verb)}>✎</button><button type="button" className="icon-action danger" aria-label="Delete verb" title="Delete verb" onClick={() => { setDeleteId(verb.id); setConfirmation(''); setError(''); }}>⌫</button></td></tr>)}</tbody></table></div>}

      {formOpen ? <div className="modal-backdrop" role="presentation"><form className="modal-card verb-modal" onSubmit={saveVerb}><div className="panel-header"><div><p className="eyebrow"><span className="eyebrow-dot" /> Verb forms</p><h2>{editingId ? 'Edit verb' : 'Add verb'}</h2></div><button type="button" className="modal-close" onClick={closeForm}>Back</button></div><div className="verb-form-grid">{fields.map((field) => <label key={field.key}><span>{field.label}</span><input value={form[field.key]} onChange={(event) => updateField(field.key, event.target.value)} required /></label>)}</div>{error ? <p className="form-error">{error}</p> : null}{result ? <p className="form-success">{result}</p> : null}<div className="form-actions"><button type="submit" className={`button primary ${working ? 'is-working' : ''}`} disabled={working}>{working ? 'Saving...' : 'Submit'}</button><button type="button" className="button secondary" onClick={clearForm} disabled={working}>Clear</button>{editingId ? <button type="button" className="button danger-button" onClick={() => { setDeleteId(editingId); setConfirmation(''); }} disabled={working}>Delete</button> : null}</div>{result ? <div className="action-result"><button type="button" className="button primary" onClick={openAdd}>Add more verbs</button><button type="button" className="button secondary" onClick={closeForm}>Back</button></div> : null}</form></div> : null}
